...
const lifeOrDeath = (x, y, surrounds, arr) => {
  ...
  let {era, age, color} = colorNAge(alive, cell, 0, 0)

  return {alive, color, key: `${x}${y}`, age, era}
}


const colorNAge = (alive, cell, age, era) => {

  if (alive > 0) {
    if (cell.age < colors0.length - 1) (age = cell.age + 1) && (era = cell.era)
    else if (cell.age === colors0.length - 1) era = cell.era + 1
  }
