...
export const newBoard = (arr, rules) => {
  ...
  if ((rules === '2') && eonPos.length) {
    // let num = eonPos.length
    eonId(newArr, eonPos)
    // if (num != eonPos.length) eonId(newArr, eonPos)
    ...
    // if (!eonPos[i].id){
      ...
      for (let j = 0; j < sur.length; j++) {
        if (sur[j].eonId) { //} && !cellId) { // is taking first a mistake?
          ...
          let [x, y] = sur[j].key.split(' ').map(Number)
          newArr[x][y].eonId = Math.floor(Math.random() * 100) // ????
          eonPos.push({xy: newArr[x][y].key, id: newArr[x][y].eonId, eon: newArr[x][y].eon})
          eonId(newArr, eonPos)
        }
      }
      if (!cellId) { // no surrounds, give me id
...
const eonMove = (newArr, eonPos) => {
  ...
  for (let i = 0; i < eonPos.length; i++) {
    ...
    // if (newArr[x][y].eon > 2) {
      ...
      newArr[X][Y] = {...newArr[x][y]}  //deepClone(newArr[x][y]) // spread seems to work for shallow
      newArr[X][Y].key = `${X} ${Y}`
