...
export const newBoard = (arr, rules) => {
  ...
  if ((rules === '2') && eonPos.length) {
    ...
    // then assign a direction to each formation
    // then sort order appropriately
...
const eonId = (newArr, eonPos) => {
  ...
  for (let i = 0; i < eonPos.length; i++) {
      ...
      let [x, y] = eonPos[i].xy.split(' ').map(Number) // eww
      ...
      if (!newArr[x][y].eonId) { // no surrounds, give me id
        newArr[x][y].eonId = Math.floor(Math.random() * 100) // if this id is not already in use?
...
const eonMove = (newArr, eonPos) => {
  ...
  for (let i = 0; i < eonPos.length; i++) {
    let [x, y] = eonPos[i].xy.split(' ').map(Number)
      ...
      newArr[X][Y] = {...newArr[x][y]} // spread seems to work for shallow
      newArr[X][Y].key = `${X} ${Y}`
