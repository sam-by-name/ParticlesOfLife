...
const lifeOrDeath = (x, y, surrounds, arr) => {
  ...
  let {age, era, eon, color} = colorNAge(alive, cell, 0, 0, 0)

  return {alive, color, key: `${x}${y}`, age, era, eon}
}


const colorNAge = (alive, cell, age, era, eon) => {
  ...
  if (alive > 0) {
    ...
    // Era above, Eon below
    if ((era > 0) && (era % 10 === 0) && age === 0) { // This is not right
      eon = cell.eon + 1 // add something to determine if alive or not
    } else eon = cell.eon
  }
