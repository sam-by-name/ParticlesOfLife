...
const lifeOrDeath = (x, y, surrounds, arr) => {
  ...
  let {age, era, eon, color, li} = colorNAge(alive, cell, 0, 0, 0)

  return {alive: li, color, key: `${x}${y}`, age, era, eon}
}
...
const colorNAge = (alive, cell, age, era, eon) => {
  ...
  if (alive > 0) {
    ...
    // Era above, Eon below
    if ((era > 0) && (era % 10 === 0) && age === 0) {
      eon = cell.eon + 1
      alive = 2 // attempt at giving life after long survival
    } else eon = cell.eon
  }
