const colors0 = [
  '#000000',  '#000099', '#0000b3', '#0000cc', '#0000e6',
  '#0000ff', '#1a1aff', '#3333ff', '#4d4dff', '#6666ff',
  '#8080ff', '#9999ff', '#b3b3ff', '#ccccff', '#e6e6ff', '#ffffff'
]

const colors1 = [
  '#f2e6ff', '#e6ccff', '#d9b3ff', '#cc99ff', '#bf80ff', '#b366ff', '#a64dff',
  '#9933ff', // middle
  '#a64dff', '#b366ff', '#bf80ff', '#cc99ff', '#d9b3ff', '#e6ccff', '#f2e6ff',
  '#ffffff' // last is 
]

export const createLife = (xy, boo) => {
  let arr = []
  
  for (let i = 0; i < xy; i++) {
    let temp = []
    for (let j = 0; j < xy; j++) {
      temp.push({
        alive: 0,
        color: colors0[0],
        key: [i, j],
        age: 0,
        era: 0,
        eon: 0,
        eonId: 0
      })
    }
    arr.push(temp)
  }
  if (boo) return makeBlock(arr) // randomizeLife(arr)
  else return arr
}

const makeBlock = (arr) => {
  for (let i = 43; i < 45; i++) {
    for (let j = 43; j < 45; j++) {
      arr[i][j].alive = 1
      arr[i][j].color = colors0[1]
      arr[i][j].age = 1
      arr[i][j].era = 8
      arr[i][j].eon = 1
    }
  }
  return arr
}

export const randomizeLife = arr => {
  for (let i = 1; i < arr.length; i++) {
    for (let j = 1; j < arr.length; j++) {
      // if (i === 1 && j === 2) (arr[i][j].alive = 1) && (arr[i][j].color = 'white')      // glider
      // else if (i === 2 && j === 3) (arr[i][j].alive = 1) && (arr[i][j].color = 'white') // glider
      // else if (i === 3) (arr[i][j].alive = 1) && (arr[i][j].color = 'white')            // glider
      arr[i][j].alive =  Math.random() > 0.85 // (Math.random()>0.5) ? 1 : 0 
      arr[i][j].color = colors0[arr[i][j].age]
      arr[i][j].age = arr[i][j].alive ? 1 : 0
    }
  }
  return arr
}

// and fix var names (everywhere)
export const newLife = (last, next, rules, raf, gen) => {
  let eonPos = []
  for (let x = 0; x < last.length; x++) {
    for (let y = 0; y < last.length; y++) {
      lifeOrDeath(last[x][y], next[x][y],
        findSur(x, y, last).reduce((a, c) => a + c.alive, 0)
      )
      colorNAge(last[x][y], next[x][y], rules)
      next[x][y].eonId = next[x][y].alive ? last[x][y].eonId : 0
      if ((next[x][y].eon > 1) && (rules === '2')) {
        eonPos.push({xy: [x, y], id: next[x][y].eonId, eon: next[x][y].eon})
      }
    }
  }
  if ((rules === '2') && eonPos.length) {
    eonId(next, eonPos)
    eonPos = eonSort(eonPos, next.length)
    eonMove(next, eonPos) // should happen before board/gen edit, and not move every gen
  }

  return {lifeA: next, lifeB: last, eonPos, raf, gen}
}

const findSur = (x, y, last) => {
  let max = last.length

  let tR = x - 1 < 0 ? (max - 1) : x - 1
  let bR = (x + 1 === max) ? 0 : x + 1
  let lC = y - 1 < 0 ? (max - 1) : y - 1
  let rC = (y + 1 === max) ? 0 : y + 1

  return [
    last[tR][lC], last[tR][y], last[tR][rC],
    last[x][lC], last[x][rC],
    last[bR][lC], last[bR][y], last[bR][rC]
  ]
}

const lifeOrDeath = (last, next, sur) => {
  if (last.alive) {
    if (sur < 2 || sur > 3) next.alive = 0
    else next.alive = 1
  } 
  if (!last.alive) {
    if (sur === 3) next.alive = 1
    else next.alive = 0
  }
}


// eon = 5 era = 80 age/gen ... for now
const colorNAge = (last, next, rules) => {
  if (next.alive) {
    if (last.age < colors0.length - 1) {
      next.age = last.age + 1
      next.era = last.era
    } else {
      next.age = 0
      next.era = last.era + 1
    }
    // Era above, Eon below
    if ((next.era) && (next.era % 5 === 0) && !next.age) {
      next.eon = last.eon + 1
      if (rules === '1') next.alive = 2 // attempt at giving life after long survival
    } else {
      next.eon = last.eon
    }
  } else {
    next.age = 0
    next.era = 0
    next.eon = 0
  }
  next.era > 0
    ? next.color = colors1[next.age]
    : next.color = colors0[next.age]
}


// life forms can have the same id as another, do I care?
// if rules'2' and newArr contains particle that has reached 2 eons
// Find a way to improve / make more efficient
// Can I rework to remove eonId from newArr
const eonId = (next, eonPos) => {
  for (let i = 0; i < eonPos.length; i++) {

    let [x, y] = eonPos[i].xy
    let sur = findSur(x, y, next)

    for (let j = 0; j < sur.length; j++) {
      if (sur[j].eonId) {                                  // if surround had eonId
        next[x][y].eonId = sur[j].eonId                    // so to does current
        eonPos[i].id = sur[j].eonId
      } 
      else if (sur[j].eon === 1) {                         // if surround is eon 1 but not yet 2
        let [r, c] = sur[j].key
        next[r][c].eonId = Math.floor(Math.random() * 100) // give it an eonId
        eonPos.push({xy: [r, c], id: next[r][c].eonId, eon: next[r][c].eon})
        eonId(next, eonPos)                                // recursion to collect all of the life form
      }
    }
    if (!next[x][y].eonId) { // no surrounds, give me id
      next[x][y].eonId = Math.floor(Math.random() * 100)
      eonPos[i].id = next[x][y].eonId
    }
  }
}


const eonMove = (newArr, eonPos) => {
  // blinkers will die as of now
  for (let i = 0; i < eonPos.length; i++) {
    let [x, y] = eonPos[i].xy

    let X = eonPos[i].dir[0]
    let Y = eonPos[i].dir[1]

    newArr[X][Y] = {...newArr[x][y]}
    newArr[X][Y].key = [X, Y]

    newArr[x][y] = {
      alive: 0,
      color: colors0[0],
      key: [x, y],
      age : 0,
      era: 0,
      eon: 0,
      eonId: 0
    }

      eonPos[i].xy = [X, Y]
  }
  return newArr
}

// create function to separate by id 
// then assign a direction to each formation / group
// then sort order according to direction, so as to not override parts of themselves
// according to size, determine velocity
// Then edit eonMove
const eonSort = (eonPos, max) => { // has to be a cleaner way to do all this
  let tR, lC, bR, rC
  let num = 0
  let arr = []
  eonPos.sort((a, b) => {return a.id - b.id})
  
  let [x, y] = eonPos[0].xy
  x = x - 1 < 0 ? (max - 1) : x - 1
  y = y - 1 < 0 ? (max - 1) : y - 1
  let temp = [eonPos[0]]
  temp[0].dir = [x, y]
  
  for (let i = 1; i < eonPos.length; i++) { // still breaking at the edges?
    [x, y] = eonPos[i].xy

    tR = x - 1 < 0 ? (max - 1) : x - 1
    bR = (x + 1 === max) ? 0 : x + 1
    lC = y - 1 < 0 ? (max - 1) : y - 1
    rC = (y + 1 === max) ? 0 : y + 1
    
    let dir = [
      [tR, lC], [tR, y], [tR, rC],
      [x, lC], [x, rC],
      [bR, lC], [bR, y], [bR, rC]
    ]
    if (eonPos[i].id === eonPos[i - 1].id) {
      eonPos[i].dir = dir[num] // re-use id to store direction because id no longer needed?
      temp.push(eonPos[i])
    }
    else {
      sortTemp(temp, num, max) // try's to sort by order needed to be moved without overwriting self
      arr.push(...temp)   // flatten

      num < 7 ? num++ : num = 0

      eonPos[i].dir = dir[num]
      temp = [eonPos[i]]
    }
  }
  sortTemp(temp, num, max)
  arr.push(...temp)       // flatten

  return arr
}

const sortTemp = (temp, num, max) => { // breaks at edges
  if (num < 3) {
    if (edge(temp, (max - 1)) && edge(temp, 0)) {     // if at or over edge / corner 
      temp.sort((a, b) => {return a.xy[0] + b.xy[0]})      // botRow   first to move
    } else temp.sort((a, b) => {return a.xy[0] - b.xy[0]}) // topRow   first to move
  }
  else if (num < 4) {
    if (edge(temp, (max - 1))) {
      temp.sort((a, b) => {return b.xy[1] - a.xy[1]})      // rightCol first to move
    } else temp.sort((a, b) => {return a.xy[1] - b.xy[1]}) // leftCol  first to move
  }
  else if (num < 5) {
    if (edge(temp, 0)) {
      temp.sort((a, b) => {return a.xy[1] - b.xy[1]})      // leftCol  first to move
    } else temp.sort((a, b) => {return b.xy[1] - a.xy[1]}) // rightCol first to move
  }
  else if (num < 8) {
    if (edge(temp, 0)) {
      temp.sort((a, b) => {return a.xy[0] - b.xy[0]})      // topRow   first to move
    } else temp.sort((a, b) => {return a.xy[0] + b.xy[0]}) // botRow   first to move
  }
}

const edge = (temp, edge) => {
  let boo = false
  for (let i = 0; i < temp.length; i++) {
    let [x, y] = temp[i].xy 
    if ((x === edge) || (y === edge)) {
      boo = true
      break
    }
  }
  return boo
}