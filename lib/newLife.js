...
const id = (next, eonPos) => {
  for (let i = 0; i < eonPos.length; i++) {
    ...
    for (let j = 0; j < sur.length; j++) {  //  WE ARE GETTING DOUBLES AND THAT IS NOT WHAT I WANT!!!!!!!!!!!!!!!!!!!!!!!!
      if (sur[j].id) {                               // if surround had id
        next[x][y].id = sur[j].id                    // so to does current
        eonPos[i].id = sur[j].id                     // do I need both?
      } 
      else if (sur[j].alive) {   // .eon > 0 swapped for alive     // if surround is eon 1 but not yet 2
        let [r, c] = sur[j].key
...
const eonSort = (eonPos, max) => { // has to be a cleaner way to do all this
  eonPos = eonDir(eonPos)
  let tR, lC, bR, rC, x, y, dir

  for (let i = 0; i < eonPos.length; i++) { // still breaking at the edges?
    for (let j = 0; j < eonPos[i].form.length; j++) {
      [x, y] = eonPos[i].form[j].key
      ...
      eonPos[i].form[j].dir = dir[eonPos[i].dir]
...
const eonDir = (eonPos) => {
  ...
  let arr = []
  let temp = {dir: num, form: [eonPos[0]]}
