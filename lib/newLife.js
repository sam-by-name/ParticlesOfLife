...
export const createLife = (x, y, boo) => {
  let arr = []
  for (let i = 0; i < x; i++) {
    let temp = []
    for (let j = 0; j < y; j++) {
...
export const randomizeLife = arr => {
  for (let i = 1; i < arr.length; i++) {
    for (let j = 1; j < arr[0].length; j++) {
      ...
      if (arr[i][j].alive) arr[i][j].opacity = 0
    }
  }
  return arr
}

export const fade = (last, next, boo, func) => {
  let max = 0, a, b
  for (let i = 1; i < next.length; i++) {
    for (let j = 1; j < next[0].length; j++) {
      if (boo && last[i][j].alive) {
        a = Math.random() + .5
        b = Math.random() + .5
        next[i][j].alive = 1
        next[i][j].age = last[i][j].age
        next[i][j].color = colors0[last[i][j].age]
        next[i][j].opacity = 1
...
export const findSur = (x, y, last) => {
  let maxH = last.length
  let maxL = last[0].length

  let tR = x - 1 < 0 ? (maxH - 1) : x - 1
  let bR = (x + 1 === maxH) ? 0 : x + 1
  let lC = y - 1 < 0 ? (maxL - 1) : y - 1
  let rC = (y + 1 === maxL) ? 0 : y + 1
...
const applyRules = (rules, next, eonPos, eonTemp) => {
  ...
  for (let i = 0; i < eonPos.length; i++) {   // place all copies of the moving life forms in their new place
    for (let j = 0; j < eonPos[i].form.length; j++) {
      if (!eonPos[i].move) {
        ...
        eonPos[i].num = 0
        if (j === eonPos[i].form.length - 1) { // decides how often the form moves
...
export const borderRad = (next) => {
  for (let x = 0; x < next.length; x++) {
    for (let y = 0; y < next[x].length; y++) {
      let tl, tr, br, bl, bg
